...
import uk.ac.cf._5.group14.BehaviourChangeGroupProject.FocusData.DailyFocusService;
...
@ActiveProfiles("test")
class CalendarDayDailyFocusEditTest {
    ...
    void dayViewRendersDailyFocusSection() throws Exception {
        ...
        mvc.perform(get("/calendar/day/2026-01-15?dailyFocus=Hydrate").sessionAttr("user", sessionUser))
                .andExpect(status().isOk())
                .andExpect(content().string(org.hamcrest.Matchers.containsString("Daily Focus")))
                .andExpect(content().string(org.hamcrest.Matchers.containsString("data-testid=\"daily-focus\"")))
                .andExpect(content().string(org.hamcrest.Matchers.containsString("data-testid=\"daily-focus-card\"")))
                .andExpect(content().string(org.hamcrest.Matchers.containsString("data-testid=\"daily-focus-description\"")))
            .andExpect(content().string(org.hamcrest.Matchers.containsString("data-testid=\"daily-focus-select\"")))
            .andExpect(content().string(org.hamcrest.Matchers.containsString("data-testid=\"daily-focus-save\"")))
            .andExpect(content().string(org.hamcrest.Matchers.containsString("value=\"Hydrate\"")));
    }

    @Test
    void postDailyFocusRedirectsBackToDayWithQueryParam() throws Exception {
        User sessionUser = new User();
        sessionUser.setId(1L);

        mvc.perform(post("/calendar/day/2026-01-15/daily-focus")
                        .param("dailyFocus", "Hydrate")
                        .sessionAttr("user", sessionUser))
                .andExpect(status().is3xxRedirection())
                .andExpect(redirectedUrl("/calendar/day/2026-01-15?dailyFocus=Hydrate"));

        verify(dailyFocusService).setDailyFocus(eq(sessionUser), eq(LocalDate.of(2026, 1, 15)), eq("Hydrate"));
    }
