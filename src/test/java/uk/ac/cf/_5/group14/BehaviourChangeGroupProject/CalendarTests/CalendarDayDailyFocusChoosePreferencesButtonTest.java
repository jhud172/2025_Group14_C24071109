...
@ActiveProfiles("test")
class CalendarDayDailyFocusChoosePreferencesButtonTest {
    ...
    void dayViewShowsChoosePreferencesButtonWhenNoPreferencesSelected() throws Exception {
        ...
        given(userPreferenceService.getUserPreferences(eq(sessionUser))).willReturn(Collections.emptyList());
        given(userPreferenceService.getUsersPhysicalConditions(eq(sessionUser))).willReturn(Collections.emptyList());
        ...
        mvc.perform(get("/calendar/day/2026-01-15").sessionAttr("user", sessionUser))
            ...
            .andExpect(content().string(org.hamcrest.Matchers.containsString("choose prefferences")))
            .andExpect(content().string(org.hamcrest.Matchers.containsString("/select-preferences#preferences")))
            .andExpect(content().string(org.hamcrest.Matchers.containsString("data-testid=\"daily-focus-select\"")));

        verifyNoInteractions(dailyFocusAiService);
    }
