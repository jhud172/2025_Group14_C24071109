...
class UserSettingsCalendarOrderingPreferencePersistenceTest {
    ...
    void updateCalendarPreferencesPersistsOrdering() {
        ...
        userSettingsService.updateCalendarPreferences(user, CalendarTaskOrderingPreference.ALPHABETICAL, null);

        UserSettings reloaded = userSettingsRepository.findById(user.getId()).orElseThrow();
        assertThat(reloaded.getCalendarTaskOrdering()).isEqualTo(CalendarTaskOrderingPreference.ALPHABETICAL);
    }
}
