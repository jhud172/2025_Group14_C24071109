...
import uk.ac.cf._5.group14.BehaviourChangeGroupProject.CalendarData.TaskTemplate;
...
@ActiveProfiles("test")
class CalendarDayTemplatesRenderTest {
    ...
    void dayViewRendersTemplateSections() throws Exception {
        ...
        tpl.setId(10L);
        tpl.setTitle("Make bed");
        tpl.setNotes("Quick win");
        tpl.setExercise(false);
        ...
        given(taskTemplateService.listRecents(eq(sessionUser), eq(6))).willReturn(List.of(tpl));
        given(taskTemplateService.listFavourites(eq(sessionUser))).willReturn(Collections.emptyList());
        given(taskTemplateService.listAll(eq(sessionUser))).willReturn(List.of(tpl));

        mvc.perform(get("/calendar/day/2026-01-15").sessionAttr("user", sessionUser))
                .andExpect(status().isOk())
                .andExpect(content().string(org.hamcrest.Matchers.containsString("Templates")))
                .andExpect(content().string(org.hamcrest.Matchers.containsString("Recents")))
                .andExpect(content().string(org.hamcrest.Matchers.containsString("Favourites")))
                .andExpect(content().string(org.hamcrest.Matchers.containsString("All")))
                .andExpect(content().string(org.hamcrest.Matchers.containsString("Make bed")));
    }
