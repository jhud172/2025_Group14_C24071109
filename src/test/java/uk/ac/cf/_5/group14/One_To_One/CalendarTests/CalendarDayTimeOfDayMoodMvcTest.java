...
import uk.ac.cf._5.group14.One_To_One.FocusData.TimedFocus;
...
@ActiveProfiles("test")
class CalendarDayTimeOfDayMoodMvcTest {
    ...
    @Test
    void dayViewAddsMiddayMoodClasses() throws Exception {
        assertMood("Midday", "to-slate-200");
    }

    @Test
    void dayViewAddsEveningMoodClasses() throws Exception {
        assertMood("Evening", "from-slate-100");
    }

    @Test
    void dayViewAddsNightMoodClasses() throws Exception {
        assertMood("Night", "to-slate-300");
    }

    private void assertMood(String label, String expectedClassFragment) throws Exception {
        User sessionUser = new User();
        sessionUser.setId(1L);
        LocalDate date = LocalDate.of(2026, 1, 15);

        given(timedFocusService.getTimedFocus()).willReturn(new TimedFocus(label));
        ...
        mvc.perform(get("/calendar/day/2026-01-15").sessionAttr("user", sessionUser))
                ...
                .andExpect(content().string(org.hamcrest.Matchers.containsString("bg-gradient-to-b")))
                .andExpect(content().string(org.hamcrest.Matchers.containsString(expectedClassFragment)));
    }
